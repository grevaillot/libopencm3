...
enum rcc_osc rcc_system_clock_source(void)
{
	switch ((RCC_CFGR >> RCC_CFGR_SWS_SHIFT) & RCC_CFGR_SWS_MASK) {
		case RCC_CFGR_SWx_HSI16:
			return RCC_HSI16;
		case RCC_CFGR_SWx_HSE:
			return RCC_HSE;
		case RCC_CFGR_SWx_PLL:
			return RCC_PLL;
		default:
			cm3_assert_not_reached();
			return 0;
...
void rcc_wait_for_sysclk_status(enum rcc_osc osc)
{
	...
	switch (osc) {
		case RCC_PLL:
			sws = RCC_CFGR_SWx_PLL;
			break;
		case RCC_HSE:
			sws = RCC_CFGR_SWx_HSE;
			break;
		case RCC_HSI16:
			sws = RCC_CFGR_SWx_HSI16;
			break;
		default:
			cm3_assert_not_reached();
			break;
	...
	uint32_t pllq, uint32_t pllr)
{
	RCC_PLLCFGR = (source << RCC_PLLCFGR_PLLSRC_SHIFT) |
		...
		(pllp << RCC_PLLCFGR_PLLPDIV_SHIFT) |
